                    ...
                    if check_response.status_code == 307:  # valid bucket, lets check if its public
                        ...
                        if not ARGS.only_interesting or (ARGS.only_interesting and any(keyword in bucket_response.text for keyword in KEYWORDS)):
                            ...
                            if bucket_response.status_code == 200:
                                ...
                                print(colored("%s is public%s" % (new_bucket_url, ", owned by " + bucket_owner if bucket_owner else ""), 'green', attrs=['bold']))
                                ...
                                if self.can_access_bucket(bucket_name):
                                    bucket_owner = self.get_bucket_owner(bucket_name)
                                    print(colored("%s is authenticated%s" % (new_bucket_url, ", owned by " + bucket_owner if bucket_owner else ""), 'magenta', attrs=['bold']))
                                    self.log(new_bucket_url)
                except:
                    pass
                finally:
    ...
    def listen(message, context):
        ...
        if len(CHECKED_BUCKETS) % 100 == 0:
            print(colored('%s buckets checked. %s buckets found' % (len(CHECKED_BUCKETS), FOUND_COUNT), 'cyan'))
    ...
    def main():
        ...
        if not CONFIG['aws_access_key'] or not CONFIG['aws_secret']:
            print(colored('Enter AWS keys in config.yaml to find even more buckets!', 'yellow', 'on_white'))

        for _ in range(1, ARGS.threads):
            BucketWorker(BUCKET_QUEUE).start()

        print(colored('Waiting for certstream events - this could take a few minutes to queue up...', 'yellow', attrs=['bold']))
        certstream.listen_for_events(listen) #blocking

        print(colored('Qutting - waiting for threads to finish up...', 'yellow', attrs=['bold']))
        BUCKET_QUEUE.join()
